        ...
        <div className="h-screen flex flex-col bg-[#FAFAFA]">
            ...
            <div className="flex-1 flex overflow-hidden">
                <div className="flex-1 p-6 overflow-hidden flex flex-col">
                    <div className="bg-white border border-gray-200/80 rounded-2xl shadow-sm overflow-hidden flex flex-col h-full ring-1 ring-black/2">
                        <div className="grid grid-cols-7 border-b border-gray-100 bg-white shrink-0">
                            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                                <div key={day} className="py-3 px-3 text-right text-[11px] font-bold text-gray-500 uppercase tracking-widest border-r border-gray-100 last:border-r-0">
                                    {day}
                                </div>
                            ))}
                        </div>
                            ...
                            {Array.from({ length: daysInMonth }).map((_, index) => {
                                ...
                                return (
                                    <div key={day} className={`p-1.5 border-r border-b border-gray-100 transition-colors flex flex-col group relative ${isCurrentDay ? 'bg-blue-50/20' : 'hover:bg-gray-50/60'}`}>
                                        {isCurrentDay && <div className="absolute top-0 left-0 w-full h-0.5 bg-blue-500"></div>}
                                            ...
                                            {dayTasks.map(task => {
                                                ...
                                                return (
                                                    <div 
                                                        ...
                                                        className={`px-2 py-1.5 border-l-[3px] rounded-r border border-transparent shadow-[0_1px_2px_rgba(0,0,0,0.02)] cursor-pointer transition-all flex items-center gap-1.5 ${getStatusColor(task.status, task)} group/task hover:-translate-y-px hover:shadow-md mb-1 last:mb-0`}
                                                        title={task.title}
                                                    >
                                                        ...
                                                        {assignee && (
                                                            assignee.avatar ? (
                                                                ...
                                                                <div className="w-4 h-4 rounded-full bg-linear-to-br from-blue-400 to-purple-500 text-white flex items-center justify-center text-[7px] font-bold shrink-0 ring-1 ring-white">
