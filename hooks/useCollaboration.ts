...
export function useCollaboration(projectId: string | string[], fallbackContent: string = '') {
    ...
    // Attempt local recovery immediately
    ...
    // Refresh content when pid changes (e.g. flipping pages)
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem(storageKey);
    ...
    // Sub to Broadcast Channel for Multi-Tab Sync
    useEffect(() => {
        if (!pid) return;
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (payload.userId === user?._id) return; // ignore own

            if (type === 'document-update') {
               setContent(payload.content); // receive updates
    ...
    const saveVersion = useCallback((snapshotContent: string) => {
        ...
        // Add to global activity feed (if using an Activity Store)
