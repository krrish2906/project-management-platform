...
export default function BacklogPage() {
    ...
    const getTasksForSprint = (sprintId: string | null) => {
        ...
        }).sort((a, b) => a.order - b.order); // Basic ordering
    };
    ...
    const onDragEnd = async (result: DropResult) => {
        ...
        // If dragged across droppables (e.g. from backlog to sprint or vice versa)
        ...
        // In a real app we'd also handle `order` updates here for same-list reordering
    };
    ...
    const handleConfirmCompleteSprint = async () => {
        ...
        // In a production app, we might do this in a single bulk API call
                ...
                >
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="w-16">
                            <input 
                                type="number"
                                className="w-full text-center text-xs font-bold bg-gray-100 hover:bg-gray-200 border border-transparent hover:border-gray-300 rounded px-1 py-1 outline-none focus:bg-white focus:border-blue-500 transition-all"
    ...
    const renderSprintList = (sprint: Sprint, isActive: boolean) => {
        ...
        const isExpanded = expandedSprints[sprint._id] !== false; // Default true
                    ...
                    <Droppable droppableId={sprint._id}>
                        {(provided, snapshot) => (
                            <div
                                ...
                                ref={provided.innerRef}
                                className={`min-h-[10px] ${snapshot.isDraggingOver ? 'bg-blue-50/30' : ''}`}
                            >
                                {sprintTasks.length > 0 ? (
                                    sprintTasks.map((t, i) => renderTaskItem(t, i))
                                ) : (
                                    <div className="p-8 text-center text-sm text-gray-400 border-t border-dashed border-gray-200 bg-gray-50/50">
                            ...
                            <Droppable droppableId="backlog">
                                {(provided, snapshot) => (
                                    <div
                                        {...provided.droppableProps}
                                        ref={provided.innerRef}
                                        className={`min-h-[100px] border-t border-gray-100 ${snapshot.isDraggingOver ? 'bg-blue-50/30' : ''}`}
                                    >
                    ...
                    <div className="bg-white rounded-xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-6">
                            ...
                            <form onSubmit={handleCreateSprint}>
                                <div className="mb-6">
                                    <label className="block text-sm font-bold text-gray-700 mb-2">Sprint Name</label>
                                    <input 
                                        autoFocus
                                        required
                                        type="text"
