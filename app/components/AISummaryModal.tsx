...
export default function AISummaryModal({
    ...
    const renderMarkdown = (text: string) => {
        return text
            .split('\n')
            .map((line, i) => {
                // Process bold text first so it works inside headings
                ...
                if (processed.startsWith('- ') || processed.startsWith('* ')) {
                    return `<li class="ml-4 text-sm text-gray-700 leading-relaxed list-disc">${processed.slice(2)}</li>`;
                }
                if (processed.trim() === '') {
                    return `<div class="h-2"></div>`;
                }
                return `<p class="text-sm text-gray-700 leading-relaxed">${processed}</p>`;
            })
            .join('');
    };
            ...
            <div className="relative w-full max-w-2xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden animate-fadeIn max-h-[85vh] flex flex-col">
                ...
                {summary && !isLoading && (
                    <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between bg-gray-50/50">
                        <p className="text-[11px] text-gray-400">Generated by AI · Results may vary</p>
                        <button
                            onClick={handleCopy}
